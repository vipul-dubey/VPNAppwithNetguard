...
public class ActivityMain extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    CommonUtils utilityObj;
    DeviceAdminUtility adminObj;
    protected int UniqueCode = 0;
    private boolean mCheckInOutVal = false;
    MSASQLiteDB dbObj = null;
    AndroidVersionRepository currentOS;
    ...
    public static final String ACTION_RULES_CHANGED = "inc.bitwise.vpnapp.ACTION_RULES_CHANGED";
    public static final String ACTION_QUEUE_CHANGED = "inc.bitwise.vpnapp.ACTION_QUEUE_CHANGED";
    ...
    protected String TapInType = "";
    public static String BtAddress;
    public static String inOutAction = "";
    public static String facility = "";
    TextView showMessageLabel = null;
...
*/
        ...
        //*****************Start  Code For TapIn Out ****************************//
        dbObj = new MSASQLiteDB(this.getApplicationContext());
        utilityObj = new CommonUtils(this.getApplicationContext());
        currentOS = AndroidVersionRepository.newInstance();
        showMessageLabel = findViewById(R.id.lblShowMessage);

        dbObj.PutLog("[InOut]onCreate");
        Intent myIntent = getIntent();
        if(myIntent.hasExtra("TapInType") && myIntent.hasExtra("mCheckInOutVal") ) {
            currentOS.StopScan();

            TapInType = myIntent.getStringExtra("TapInType");
            mCheckInOutVal = myIntent.getBooleanExtra("mCheckInOutVal", true);
            dbObj.PutLog("[InOut]token::"+TapInType+"mCheckInOutVal::"+mCheckInOutVal);
            Log.i("MSA","[InOut]token::"+TapInType+ "mCheckInOutVal::"+mCheckInOutVal);
        }
        ...
        // On/off switch
        swEnabled.setClickable(false);
        swEnabled.setChecked(mCheckInOutVal);
       // enableVPN(prefs,enabled);
        swEnabled.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                enableVPN(prefs,isChecked);
            }
        });
        if (enabled)
            checkDoze();

        //VPN Service Start Stop On Tap in/out
        enableVPN(prefs,mCheckInOutVal);
        ...
        tvDisabled.setVisibility(mCheckInOutVal ? View.GONE : View.VISIBLE);

        // Application list ***********************commented****************************************
    ...
    private void ValidateDeviceAdmin() {
        adminObj = new DeviceAdminUtility(this.getApplicationContext());
        Intent deviceAdminIntent = null;
        try {
            deviceAdminIntent = adminObj.ValidateDeviceAdminPolicyStatus();
        } catch (Exception e) {
                    e.printStackTrace();
        }

        if (deviceAdminIntent != null)
            startActivityForResult(deviceAdminIntent, REQUEST_ENABLE);

        ProceedINOUTActivity();
    }

    private void ProceedINOUTActivity() {
        try {
            currentOS.DisplayPopUpForRequiredPermission(ActivityMain.this, this.getApplicationContext());
            GetInOutActionAndFacilityFromNFC();
            if (inOutAction.equals("out")) // while out
            {
                Deletefile();
                ClearOldLogsOnTapOut();
            }
                RequestInOutAPI();
        }
        catch (Exception e) {
            dbObj.PutLog("[@@InOut]EX ProceedINOUT - " + e.toString());
        }

    }

    private void RequestInOutAPI() {
        if (currentOS.CheckRequiredPermissionsByApp(ActivityMain.this, this.getApplicationContext()))
            Toast.makeText(this, "[SMS][Phone][Contact][Storage]Permissions are mandatory, don't Deny", Toast.LENGTH_LONG).show();

        try {
            if (!utilityObj.isRooted()) {
                if (inOutAction.equals("in")) {
                    SharedPreferences sharedPreferences = ActivityMain.this.getSharedPreferences(SHARED_PREFS, ActivityMain.this.MODE_PRIVATE);
                    //text = sharedPreferences.getString(TEXT, "");
                    String username = sharedPreferences.getString(Username, "");
                    String tapValue = mCheckInOutVal ? "In" : "Out";

                    utilityObj.TapIntrackingUpdate(username,tapValue);
                    adminObj.DisableCamera();
                    dbObj.PutLog("[InOut]SAFE Compliant");
                    utilityObj.sendNotification(getString(R.string.SafeCompliant));
                    showMessageLabel.setText(getString(R.string.SafeCompliant));
                    showMessageLabel.setTextColor(Color.parseColor("#ff0000"));
                    showMessageLabel.setVisibility(View.VISIBLE);
                    Toast.makeText(this, "Camera is Disabled.", Toast.LENGTH_SHORT).show();

                    final Intent myIntent = new Intent(this, WakeScreenService.class);
                    ...
                    adminObj.EnableCamera();
                    Toast.makeText(this, "Camera is Enabled.", Toast.LENGTH_SHORT).show();
                    showMessageLabel.setText(getString(R.string.PersonalAgain));
                    dbObj.PutLog("[InOut]Personal again");
                    showMessageLabel.setVisibility(View.VISIBLE);
                    Thread thread = new Thread() {
                        public void run() {
                            try {
                                currentOS.StopServicesAtTAPOutAsPerOSVersion(ActivityMain.this);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                ...
                showMessageLabel.setText(getString(R.string.RootedDeviceText));

        } catch (Exception e) {
            dbObj.PutLog("[InOut]EX RequestTapINOUTService " + e.toString());
        } finally {
            if (adminObj != null)
                adminObj = null;
            if (utilityObj != null)
    ...
    public void onBackPressed() {
        ...
        /*if (!inOutAction.equals("in")) {
            try {
                Intent intent = new Intent(this,WakeScreenService.class);
        ...
        //Intent activityIntent = new Intent(getApplicationContext(), TapIn.class);
        //startActivity(activityIntent);
        finish();
    }

    public void ClearOldLogsOnTapOut() {
        try {
            //GEt current date then calculate 2 days previous date
            ...
            // Clear Logs From Device which are older then previousDate value
            dbObj.ClearLogsFromDevice(previousDate);
        }
        catch (Exception ex)
        {
            dbObj.PutLog("[@@InOut]ClrLogs " + ex.toString());
        }
    }
    ...
    private void Deletefile() {
        try {
            ...
            File apkFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/BitwiseMobile.apk");
            File apkFileondownload = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/Download/BitwiseMobile.apk");

            if (apkFile.exists()) {
                boolean deleted = apkFile.delete();
                if (deleted) {
                    dbObj.PutLog("[InOut]Successfully deleted ");
                } else {
                    dbObj.PutLog("[InOut]Not Deletefile");
                }
            }

            if (apkFileondownload.exists()) {
                dbObj.PutLog("[InOut]file exists on download");
                boolean deleted = apkFileondownload.delete();
                if (deleted) {
                    dbObj.PutLog("[InOut]Successfully deleted from download");
                } else {
                    dbObj.PutLog("[InOut]Not Deletefile");
            ...
            dbObj.PutLog("[InOut]EX Delete " + e.toString());
        }
    }

    public String GetInOutActionAndFacilityFromNFC() throws Exception {
        try {
            String nfcData;
            Log.i("MSA", "inside GetInOutActionAndFacilityFromNFC" + TapInType);
            if (TapInType.equals("Bluetooth")) {
                CommonUtils.UDID = BtAddress;
                if (mCheckInOutVal == false) {
                    inOutAction = "out";
                } else if (mCheckInOutVal == true) {
                    inOutAction = "in";
                }
            } else {
                inOutAction = "in";
                ...
                Log.i("MSA", "inside GetInOutActio ELSEEEEEEEEE");

                dbObj.PutLog("[onCreate InOut] Inside NFC UID IFFFFFFF");
                dbObj.PutLog("[onCreate InOut] NFC UID IS - " + this.ByteArrayToHexString(intent.getByteArrayExtra(NfcAdapter.EXTRA_ID)));
                Log.i("MSA", "NFC UID IS" + this.ByteArrayToHexString(intent.getByteArrayExtra(NfcAdapter.EXTRA_ID)));
                String NFCUID = this.ByteArrayToHexString(intent.getByteArrayExtra(NfcAdapter.EXTRA_ID));
                CommonUtils.UDID = NFCUID;
                if (intent.getType() != null && intent.getType().equals("application/com.bitwiseglobal.nfc")) {
                    // Read the first record which contains the NFC data
                    Parcelable[] rawMsgs = intent.getParcelableArrayExtra(NfcAdapter.EXTRA_NDEF_MESSAGES);
                    NdefRecord relayRecord = ((NdefMessage) rawMsgs[0]).getRecords()[0];
                    nfcData = new String(relayRecord.getPayload());
                    facility = GetFacilityFromNFC(nfcData.trim().toLowerCase());
                    if (nfcData.contains("out")) {
                        inOutAction = "out";
                        mCheckInOutVal = false;
                    }
                }
            }
            ...
            dbObj.PutLog("[@@InOut]GetInOutActionAndFacilityFromNFC Ex "+e.toString());
    ...
    private String GetFacilityFromNFC(String facilityNFC) throws Exception {
        String newFacility = null;
        try {
            if (facilityNFC.contains("bitwise")) {
                facilityNFC = facilityNFC.replaceAll("bitwise-", "");
            } else if (facilityNFC.contains("out")) {
                facilityNFC = facilityNFC.replaceAll("-out", "");
            }

            newFacility = facilityNFC.substring(0, 1).toUpperCase() + facilityNFC.substring(1).toLowerCase();
        } catch (Exception e) {
            dbObj.PutLog("[@@InOut]GetFacility-" + e.toString());
        }
        return newFacility;
    }
    ...
    private String ByteArrayToHexString(byte [] inarray) {
        ...
        try {
            ...
            dbObj.PutLog("[@@InOut]ByteArrayToHexString Ex "+e.toString());

        }
        return out;
    }

    protected void enableVPN(final SharedPreferences prefs, boolean isChecked){
        {
...
*/
    ...
    protected void onResume() {
        ...
        // Added for Device Admin check
        DeviceAdminUtility adminObj = new DeviceAdminUtility(this.getApplicationContext());
        Intent deviceAdminIntent = null;
        try {
            deviceAdminIntent = adminObj.ValidateDeviceAdminPolicyStatus();
        } catch (Exception e) {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (adapter != null)
                if (intent.hasExtra(EXTRA_CONNECTED) && intent.hasExtra(EXTRA_METERED)) {
                    ...
                    ivIcon.setImageResource(R.drawable.ic_bitlauncher);
        ...
        try{
            new AsyncTask<Object, Object, List<Rule>>() {
                ...
                protected void onPostExecute(List<Rule> result) {
                    if (running) {
                        if (adapter != null) {
                            List<Rule> resultPredefinedLocked = new ArrayList<Rule>();

                            for(int i=0;i<result.size();i++){
                                if(result.get(i).wifi_blocked || result.get(i).other_blocked){
                                    resultPredefinedLocked.add(result.get(i));
                                }
                                else break;
                            }

                            adapter.set(resultPredefinedLocked);
                            updateSearch(search);
                        }
            ...
            Log.i("Excep ActMain","excp "+e.toString());
        }
